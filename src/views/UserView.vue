					...
					<div class="control">
						<ValidationProvider rules="required" name="Name" v-slot="{ errors, valid }">
							<div class="field">
								...
								<div class="control">
									<input id="name" type="text" class="input" v-model="localUserData.name"
										placeholder="Name" />
								</div>
					...
					<div class="control">
						<ValidationProvider rules="required" name="Email" v-slot="{ errors, valid }">
							<div class="field">
								...
								<div class="control">
									<input id="email" type="text" class="input" v-model="localUserData.email"
										placeholder="Email" />
								</div>
								<p class="help" :class="{ 'is-danger': errors[0], 'is-success': valid }">
									{{ errors[0] }}
	...
	methods: {
		...
		async updateUser() {
			try {
				...
				const response = await NetworkManager.apiRequest(
				...
				if (!response.success) {
					Object.keys(response.error).forEach(field => {
						...
						BuefyHelper.showAlert("Error", errorMessage);
					});
